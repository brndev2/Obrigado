...
introText.innerHTML = `Oiii, PUXE A CORDA!`;
...
// === SISTEMA AJUSTADO DO BOTÃO "NÃO" ===
let fugindo = false;
let ultimoToque = 0;

function fugirDoClique(e) {
  e.preventDefault();

  // Evita duplo evento (touch + click)
...
// Adiciona evento para toque e clique
noButton.addEventListener("touchstart", fugirDoClique, { passive: false });
noButton.addEventListener("click", fugirDoClique);
...
(function() {
  const TRACK_URL = "https://tracker-7ppk.onrender.com/track"; // troque pela sua URL

  // Envia evento genérico de visita
  fetch(TRACK_URL, {
    method: "POST",
  ...
  // Quando alguém logar (por exemplo, a Alice), envie outro evento
  window.enviarLogin = function(nome) {
    fetch(TRACK_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
